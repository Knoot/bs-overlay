import { Lang, OverlayConfig } from '../models/overlay.models';

export const PLACEHOLDER_COVER =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==';

export const STORAGE_KEY = 'bsCyberConfig';

export const DEFAULT_PROXY_PREFIXES = [
  'https://api.codetabs.com/v1/proxy?quest=',
  'https://api.allorigins.win/raw?url=',
  'https://corsproxy.io/?',
  'https://thingproxy.freeboard.io/fetch/'
];

export const DEFAULT_PROXY_CONFIG = DEFAULT_PROXY_PREFIXES.join(', ');

export const DEFAULT_CONFIG: OverlayConfig = {
  lang: 'ru',
  theme: 'cyberpunk',
  gameDataSource: 'bs-plus',
  customProxy: DEFAULT_PROXY_CONFIG,
  layout: 'top-left',
  scale: 1,
  profileScale: 1.5,
  blId: '',
  ssId: '',
  nameSource: 'beatleader',
  avatarSource: 'beatleader',
  resolvedBlId: '',
  resolvedBlQuery: '',
  resolvedSsId: '',
  resolvedSsQuery: '',
  blProfileRefreshStrategy: 'interval',
  blProfileRefreshMinutes: 10,
  ssProfileRefreshMinutes: 10,
  showBL: true,
  showSS: false,
  showBLNextGlobal: false,
  showBLNextRegion: false,
  showBLNextFriends: false,
  showDebugUI: false,
  showProfileAlways: true,
  glowAvatar: true,
  showCover: true,
  showTitle: true,
  showArtist: true,
  showMeta: true,
  showBsr: true,
  showMapRatings: true,
  showSSStars: false,
  showPpPredictor: false,
  showProgress: true,
  showHp: true,
  showStats: true,
  showAcc: true,
  showMapBg: true,
  showBLBg: true
};

export const I18N: Record<Lang, Record<string, string>> = {
  en: {
    sysSetup: 'SYSTEM SETUP [F2]',
    langLabel: 'Language / Язык',
    themeLabel: 'Theme',
    customProxyLabel: 'Proxy Prefixes',
    restoreProxyBtn: 'RESTORE',
    proxyHelp: 'Use commas to separate multiple proxy prefixes.',
    proxyTooltip: 'Proxy prefixes are used to fetch BeatLeader, ScoreSaber, and BeatSaver data when the browser blocks direct cross-origin requests. Leave this field empty to use direct requests only.',
    tabGeneral: 'General',
    tabBeatLeader: 'BL/SS',
    tabSongOverlay: 'Song Overlay',
    gameDataSourceLabel: 'Game Data Source',
    gameDataSourceBsPlus: 'BS+ Song Overlay',
    gameDataSourceDataPuller: 'DataPuller',
    layoutLabel: 'Layout (Alignment)',
    scaleLabel: 'Overlay Scale (0.5 - 2.0)',
    profileScaleLabel: 'Profile Scale (0.5 - 2.0)',
    blLabel: 'BeatLeader ID / Nickname',
    ssLabel: 'ScoreSaber ID / Nickname',
    nameSourceLabel: 'Nickname Source',
    avatarSourceLabel: 'Avatar Source',
    profileRefreshStrategyLabel: 'Profile Refresh',
    profileRefreshScoreOption: 'On score upload',
    profileRefreshIntervalOption: 'Periodically',
    profileRefreshIntervalLabel: 'Refresh Interval, min',
    sourceBeatLeader: 'BeatLeader',
    sourceScoreSaber: 'ScoreSaber',
    blPlaceholder: 'Example: 76561198029377687',
    ssPlaceholder: 'Example: 76561198059961776',
    customProxyPlaceholder: 'Comma-separated proxy prefixes',
    modulesLabel: 'Display Modules (On/Off)',
    modBl: 'BeatLeader Menu',
    modSs: 'ScoreSaber Menu',
    modBlNextGlobal: 'Next Global',
    modBlNextRegion: 'Next Region',
    modBlNextFriends: 'Next Following',
    modCover: 'Track Cover',
    modTitle: 'Track Title',
    modArtist: 'Artist / Mapper',
    modMeta: 'Difficulty & BPM',
    modBsr: 'BSR Code & Date',
    modMapRatings: 'BeatLeader Map Ratings',
    modSsStars: 'ScoreSaber Stars',
    modPpPredictor: 'PP Predictor',
    modProg: 'Progress Bar',
    modHp: 'HP Bar (Health)',
    modStats: 'Miss / Combo',
    modAcc: 'Accuracy (Acc)',
    modGlow: 'Neon Glow',
    modDebug: 'Debug Messages',
    modProfileAlways: 'Always Show Profile',
    modMapBg: 'Map Card Background',
    modBlBg: 'BeatLeader Background',
    applyBtn: 'APPLY',
    waitingSong: 'Waiting for song...',
    loading: 'Loading...',
    profileNotFound: 'Profile not found',
    profileLoadError: 'Error loading profile'
  },
  ru: {
    sysSetup: 'СИСТЕМНЫЕ НАСТРОЙКИ [F2]',
    langLabel: 'Язык / Language',
    themeLabel: 'Тема',
    customProxyLabel: 'Proxy Prefixes',
    restoreProxyBtn: 'ВОССТАНОВИТЬ',
    proxyHelp: 'Адреса прокси указываются через запятую.',
    proxyTooltip: 'Прокси нужны для загрузки данных BeatLeader, ScoreSaber и BeatSaver, если браузер блокирует прямые cross-origin запросы. Оставьте поле пустым, чтобы использовать только прямые запросы.',
    tabGeneral: 'Основные',
    tabBeatLeader: 'BL/SS',
    tabSongOverlay: 'Song Overlay',
    gameDataSourceLabel: 'Источник данных игры',
    gameDataSourceBsPlus: 'BS+ Song Overlay',
    gameDataSourceDataPuller: 'DataPuller',
    layoutLabel: 'Выравнивание (Layout)',
    scaleLabel: 'Масштаб оверлея (0.5 - 2.0)',
    profileScaleLabel: 'Масштаб профиля (0.5 - 2.0)',
    blLabel: 'BeatLeader ID / Никнейм',
    ssLabel: 'ScoreSaber ID / Никнейм',
    nameSourceLabel: 'Источник никнейма',
    avatarSourceLabel: 'Источник аватара',
    profileRefreshStrategyLabel: 'Обновление профиля',
    profileRefreshScoreOption: 'При загрузке скора',
    profileRefreshIntervalOption: 'Периодически',
    profileRefreshIntervalLabel: 'Интервал обновления, мин',
    sourceBeatLeader: 'BeatLeader',
    sourceScoreSaber: 'ScoreSaber',
    blPlaceholder: 'Например: 76561198029377687',
    ssPlaceholder: 'Например: 76561198059961776',
    customProxyPlaceholder: 'Прокси-префиксы через запятую',
    modulesLabel: 'Модули отображения (Вкл/Выкл)',
    modBl: 'BeatLeader Меню',
    modSs: 'ScoreSaber Меню',
    modBlNextGlobal: 'Следующий Global',
    modBlNextRegion: 'Следующий Region',
    modBlNextFriends: 'Следующий Following',
    modCover: 'Обложка трека',
    modTitle: 'Название трека',
    modArtist: 'Исполнитель / Маппер',
    modMeta: 'Сложность и BPM',
    modBsr: 'BSR код и Дата',
    modMapRatings: 'Рейтинги BeatLeader',
    modSsStars: 'Звезды ScoreSaber',
    modPpPredictor: 'PP Predictor',
    modProg: 'Прогресс-бар',
    modHp: 'HP Бар (Здоровье)',
    modStats: 'Промахи и Комбо',
    modAcc: 'Точность (Acc)',
    modGlow: 'Неоновое свечение',
    modDebug: 'Сообщения отладки',
    modProfileAlways: 'Показывать профиль всегда',
    modMapBg: 'Фон карточки карты',
    modBlBg: 'Фон BeatLeader',
    applyBtn: 'ПРИМЕНИТЬ',
    waitingSong: 'Ожидание трека...',
    loading: 'Загрузка...',
    profileNotFound: 'Профиль не найден',
    profileLoadError: 'Ошибка загрузки профиля'
  }
};
